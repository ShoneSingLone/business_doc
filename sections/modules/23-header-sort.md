...
```vue
<template>
	<xTableEasy :columns="columns" :table-data="tableData" :sort-option="sortOption" border-y />
</template>
<script>
export default {
	data() {
...
```vue
<template>
	<xTableEasy :columns="columns" :table-data="tableData" :sort-option="{ sortAlways: true }" />
</template>
<script>
export default {
  data() {
...
**sortBy 值说明**：| 值 | 说明 | |----|------| | `asc` | 默认升序 | | `desc` | 默认降序 | | `""`
| 允许排序但无默认排序规则 |
