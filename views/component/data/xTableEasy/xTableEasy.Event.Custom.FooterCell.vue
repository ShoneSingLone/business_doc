...
export default async function () {
	return defineComponent({
		data() {
			return {
				mdDoc: "footer 单元格事件自定义：通过 `eventCustomOption.footerCellEvents` 自定义页脚单元格的点击事件",
				eventLogs: [],
				eventCustomOption: {
					footerCellEvents: ({ row, column, rowIndex, columnIndex }) => {
						return {
							click: (e) => {
								this.eventLogs.push(`[${new Date().toLocaleTimeString()}] footer cell click: ${column.title}`);
