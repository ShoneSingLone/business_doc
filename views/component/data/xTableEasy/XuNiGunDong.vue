<template>
	<div class="card-XuNiGunDong">
		<xMd :md="mdTips" />
		<h4>实现原理</h4>
		<div class="principle-explanation">
			<ol>
				<li><strong>只渲染可视区域数据</strong>：根据容器高度和行高计算可视区域能显示的行数，只渲染这些行及其前后的缓冲行。</li>
				<li><strong>占位符技术</strong>：使用一个高度等于所有数据行总高度的占位符元素，确保滚动条能正常工作。</li>
				<li><strong>滚动事件监听</strong>：监听滚动容器的scroll事件，根据滚动位置动态计算当前应该显示的数据范围。</li>
				<li><strong>数据切片</strong>：根据计算出的起始和结束索引，从完整数据中切片出需要显示的部分。</li>
				<li><strong>缓冲机制</strong>：为了避免滚动时频繁更新，会在可视区域前后添加一定比例的缓冲数据。</li>
			</ol>
		</div>
		<h4>核心配置</h4>
		...
		<xTableEasy :columns="columns" :table-data="tableData" :max-height="500" border-x border-y row-key-field-name="id" :virtual-scroll-option="virtualScrollOption" />
	</div>
</template>
<script lang="ts">
export default async function () {
	...
	return defineComponent({
		data() {
			return {
				mdTips: `
- 1、虚拟滚动可以高效渲染大量数据，只渲染可视区域内的数据行。
- 2、支持4000条以上数据的流畅滚动。
- 3、大幅提升大数据量场景下的性能。
`,
				tableData: generateTestData(),
				columns: [
					{ field: "id", key: "id", title: "ID", width: 100, align: "center" },
				...
				virtualScrollOption: {
					...
					scrolling: (startRowIndex, visibleStartIndex, visibleEndIndex) => {
						console.log("虚拟滚动数据范围:", { startRowIndex, visibleStartIndex, visibleEndIndex });
					}
				}
			};
		}
...
.card-XuNiGunDong {
	...
	.principle-explanation, .config-explanation, .performance-explanation {
		margin: 0 0 20px 0;
		padding: 15px;
	...
	.config-explanation ul, .performance-explanation ul {
		margin: 0;
		padding-left: 20px;
	}
	
	.principle-explanation li, .config-explanation li, .performance-explanation li {
		margin: 8px 0;
		color: #606266;
		font-size: 14px;
	}
