<template>
	<DocContentOfDemo class="x-table-easy-table-width">
		<xMd :md="mdTips" />
		<h4>固定宽度</h4>
		<xTableEasy :columns="columns" :table-data="tableData" :max-height="300" :scroll-width="800" border-x border-y />
		<h4>百分比宽度</h4>
		<xTableEasy :columns="columns" :table-data="tableData" :max-height="300" :scroll-width="'100%'" border-x border-y />
...
export default async function () {
	return {
		data() {
			return {
				mdTips: `
- 1、通过配置 scroll-width 属性，可以设置表格的宽度。
- 2、支持像素值和百分比两种单位。
- 3、当内容超过宽度时会显示水平滚动条。
`,
				apiString: `
...
| 属性 | 说明 | 类型 | 默认值 |
|------|------|------|------|
| scroll-width | 表格宽度 | Number/String | - |
`,
				tableData: [
