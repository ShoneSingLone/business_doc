...
export default async function () {
	return {
		data() {
			return {
				mdTips: `
- 1、表格支持行多选功能，通过 checkbox 列实现
- 2、通过 checkboxOption 配置多选行为和事件回调
- 3、支持自定义选中状态和禁用某些行
- 4、选中数据可用于批量操作场景
`,
				apiString: `
...
| selectedRowKeys | 选中的行 key 数组 | Array | [] |
| disabled | 禁用指定行的函数 | Function(row, rowIndex) | - |
...
| 属性 | 说明 | 类型 | 默认值 |
|------|------|------|------|
| type | 列类型，设为 "checkbox" | String | - |
| width | 列宽度 | Number | 50 |
`,
				selectedRows: [],
				columns: [
					{ type: "checkbox", key: "checkbox", title: "", width: 50 },
