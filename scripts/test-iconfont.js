...
async function test() {
	...
	const url =
		"https://www.iconfont.cn/search/index?q=card&page=1&searchType=icon&fromCollection=-1";
	await page.goto(url, { waitUntil: "networkidle", timeout: 30000 });
	await page.waitForTimeout(3000);

	// 截图看看
	await page.screenshot({
		path: "e:/ghca_code/m2o/statics/business_doc/scripts/debug-screenshot.png",
		fullPage: true
	});
	...
	for (let i = 0; i < frames.length; i++) {
		...
		try {
			...
			if (listCount > 0) {
				...
				if (items > 0) {
					const svgHtml = await frame
						.locator(".block-icon-list li:first-child svg")
						.first()
